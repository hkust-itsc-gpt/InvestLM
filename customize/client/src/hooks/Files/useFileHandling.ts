...
const useFileHandling = (params?: UseFileHandling) => {
  ...
  const handleFiles = async (_files: FileList | File[]) => {
    ...
    if (files.size > 0) {
      ...
      const confirmReplace = window.confirm('You already have a file. Do you want to replace it?');
      if (!confirmReplace) {
        return; // Return directly if user cancels
      }
      setFiles(new Map());
    }
    const fileList = [Array.from(_files)[0]];
